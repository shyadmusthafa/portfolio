...
function renderContent() {
    ...
    $('#resume-btn').href = d.profile.resumeUrl;
    ...
    const socialHTML = d.profile.social
        .map((s) => `<a href="${s.url}" target="_blank" rel="noopener noreferrer" aria-label="${s.platform}" class="social-link"><i class="bi ${s.icon}"></i></a>`)
        .join('');
    ...
    tabsContainer.innerHTML = d.skillCategories
        .map((c, i) => `<button class="skill-tab ${i === 0 ? 'active' : ''}" data-category="${c.id}"><i class="bi ${c.icon}"></i>${c.name}</button>`)
        .join('');
...
function filterBlog(category, search) {
...
function initCharts() {
    ...
    const viewsCtx = $('#chart-views').getContext('2d');
    if (charts.views)
    ...
    const trafficCtx = $('#chart-traffic').getContext('2d');
    if (charts.traffic)
    ...
    const projectsCtx = $('#chart-projects').getContext('2d');
    if (charts.projects)
...
function updateChartsTheme() {
    if ($('#analytics-panel').classList.contains('show'))
        initCharts();
}
/* ─── Back to Top ─── */
